...
public class Principal{
    ...
    public static void inicializarDadosTeste() {
        ...
        for (int i = 0; i < salas2D.size() && filme_index < filmes.size(); i++) {
            Filme filme = filmes.get(filme_index++);
            Sala2D sala = salas2D.get(i);
            LocalTime horario = horarios[random.nextInt(horarios.length)];
            LocalDate data = random.nextBoolean() ? hoje : amanha;
        
            criarSessaoComOcupacao(filme, data, horario, sala);
        }
        
        for (int i = 0; i < salas3D.size() && filme_index < filmes.size(); i++) {
            Filme filme = filmes.get(filme_index++);
            Sala3D sala = salas3D.get(i);
            LocalTime horario = horarios[random.nextInt(horarios.length)];
            LocalDate data = random.nextBoolean() ? hoje : amanha;
        ...
        for (int i = 0; i < salasImax.size() && filme_index < filmes.size(); i++) {
            Filme filme = filmes.get(filme_index++);
            SalaImax sala = salasImax.get(i);
            LocalTime horario = horarios[random.nextInt(horarios.length)];
            LocalDate data = random.nextBoolean() ? hoje : amanha;
